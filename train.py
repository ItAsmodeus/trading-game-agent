...
def train(args):
    ...
    while True:
        ...
        snap = MarketSnapshot(
            ...
            mu_60         = float(obs[22]) if len(obs) > 22 else 0.0,
            sigma_20      = float(obs[23]) if len(obs) > 23 else 0.03,
            z_score       = float(obs[24]) if len(obs) > 24 else 0.0,
            regime        = float(obs[25]) if len(obs) > 25 else 0.0,
            ma_cross      = float(obs[26]) if len(obs) > 26 else 0.0,
